...
**🔴 セッション⑩ (2026-09-16 開始) で決定した大きな方針転換: TAMSIC リニューアル**

TAmJump からの方針決定:
- **YouTube でフル尺公開に切替** (これまでのサンプル戦略を廃止)
- **サイトの有料化 (コイン経済) を廃止**、無料化して間口を広げる
- **サイトはアーティスト募集プラットフォームへ再設計**
- **将来のファンクラブ + グッズ販売** (既存 coin はクレジット転用可、1c=¥[レート] 未確定)
- **利益の一部をへき地・海外過疎地の医療支援へ** (ブランドの核)
- **TAMJ vision-map を軸に据える**: TAMSIC は TAmJump 社の「未来予想図」の裏側で動く活動として位置づけ、`https://tamjump.com/vision-map.html` を主要な世界観の入口とする
- **英語をプライマリ言語に転換** (方針 β: 英語プライマリ + JP 切替トグル)
- **告知ストーリー**: 「みなさんの応援により、TAMSIC が生まれ変わります」
- **ONE HEART Kickstarter との切り分け**: Kickstarter プロジェクト (WE ARE ALL ONE HEART) は TAMSIC サイト内で告知しない (TAmJump 社の別事業として扱う)。今後同様の依頼があっても方針変更しないこと。

詳細方針・実装フェーズ (Phase R1〜R5) は **`docs/TAMSIC_リニューアル計画書_v1.md`** を参照。

**次に着手すべき最優先タスク**:
- **Phase R1: リニューアル準備・告知** (英語版告知文作成、転用レート確定、既存 4 名の coin 残高確認、告知メール送付)
- **Phase R2: 有料化廃止 + YouTube フル尺化** (30 coin ボタン削除、release-control 判定廃止、youtubeUrl 差し替え)
- **Phase R3: 英語プライマリ化** (data-ja/data-en 反転、TAMSICLang デフォルト EN)
- **Phase R4: アーティストプラットフォーム化** (可変数対応、応募フォーム)
- **Phase R5: ファンクラブ + グッズ + 医療支援** (Stripe/Shopify 検討、寄付フロー)
- **Phase I: 運営者向け 会員数ダッシュボード** (admin.html リアルタイム表示 + 日次メール通知、Phase R1〜R2 と並行が現実的)

**旧優先タスク (リニューアル方針との整合性を再確認する必要あり)**:
- Phase H1 (closing 500通り + creatorNote): リニューアル後の便箋メール活用方針次第で継続/中止判断。当面 Phase R2 で便箋メール一旦非表示予定、Phase R5 でファンクラブ特典として復活検討時に再着手。
